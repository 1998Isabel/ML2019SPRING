...
loaded_dict = torch.load(opt.netG)
new_dict = dict()
for (k1, v1) in loaded_dict.items():
  if 'conv.1' in k1:
      new_dict[k1.replace('conv.1', 'conv1')] = v1
  elif 'norm.1' in k1:
      new_dict[k1.replace('norm.1', 'norm1')] = v1
  elif 'norm.2' in k1:
      new_dict[k1.replace('norm.2', 'norm2')] = v1
  elif 'conv.2' in k1:
      new_dict[k1.replace('conv.2', 'conv2')] = v1
  else:
      new_dict[k1] = v1
