...
parser.add_argument('--exp', default='./src/Indoor/sample', help='folder to output images and model checkpoints')
...
with torch.no_grad():
  arget, input, depth, ato = target.cuda(), input.cuda(), depth.cuda(), ato.cuda()
  ...
  for i, data in enumerate(valDataloader, 0):
    with torch.no_grad():
      if 1:
        ...
        directory = './src/Indoor/indoor/our_cvprw_test3/'
        if not os.path.exists(directory):
            os.makedirs(directory)

        name=''.join(path)
        filename='./src/Indoor/indoor/our_cvprw_test3/'+str(i)+'.png'
